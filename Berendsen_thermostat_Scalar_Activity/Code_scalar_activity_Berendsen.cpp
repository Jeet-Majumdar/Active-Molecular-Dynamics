...
void usage ( void ) {
  ...
  fprintf(stdout,"\t -tau             [real]\t\tTemperature time constant [Default: 0.01]\n");
...
void readInputData(FILE *&filename_F, int &natoms_F, int dim_F, int *&group_F, double **&box_F, double **&pos_F, double **&vels_F, double *&mass_F){
	...
	if(natoms_F != s2){ printf("Wrong Restart Fiile!!\nError::\tNumber of atoms in restart file is %d while given input is %d\n", s2, natoms_F); exit(0); }
	natoms_F = s2;
...
void variable_Initialization(double **&pos_F, double **&vels_F, int *&group_F, double **&forces_F, double **&box_F,\
	...
	double radii_F, double &ke_Energy_hot_F, double desired_temp_hot_F, double dt_F, double epsilon_F, \
	double sigma_F, double desired_temp_cold_F, double &ke_Energy_cold_F, int n_hot_atoms_F){
	...
	// Scale velocity to a given starting temperature by default
		...
		double factor_scale_vel_hot = sqrt(desired_temp_hot_F/scale_T_hot);
		double scale_T_cold = (natoms_F - n_hot_atoms_F)>0?2.0*(ke_Energy_hot_F)/(natoms_F - n_hot_atoms_F)/((double)dim_F):0.0;
		double factor_scale_vel_cold = (natoms_F - n_hot_atoms_F)>0?sqrt(desired_temp_cold_F/scale_T_cold):0.0;
		ke_Energy_hot_F = 0;	
		ke_Energy_cold_F = 0;
		for(int j=0; j<dim_F; j++){
    		for(int i=0; i<natoms_F; i++){
...
void integrate(int natoms_F, double **&pos_F, double **&vels_F, double *&mass_F, double **&force_F, \
	int *&group_F, double dt_F, double rcut_off_F, \
	double epsilon_F, double sigma_F, double &pot_Energy_F, double **&box_F, \
	double &pressure_virial_F, double energy_correction_F, double energy_at_cutoff_F){
    ...
    for(int i=0; i<natoms_F; i++){
		...
		vels_F[i][0] = vels_F[i][0] + 0.5*dt_F*force_F[i][0]/mass_F[i];
		vels_F[i][1] = vels_F[i][1] + 0.5*dt_F*force_F[i][1]/mass_F[i];
		vels_F[i][2] = vels_F[i][2] + 0.5*dt_F*force_F[i][2]/mass_F[i];
        }
...
void berendsen_thermostat(double temp_hot_F, double temp_cold_F, \
                            double desired_temp_hot_F, double desired_temp_cold_F, \
                            double tau_hot_F, double tau_cold_F, \
                            double **&vels_F, int *&group_F, \
                            int dim_F, int natoms_F, double dt_F){

    double lamda_hot = sqrt(1+dt_F/tau_hot_F*(desired_temp_hot_F/temp_hot_F - 1.0));
    double lamda_cold = sqrt(1+dt_F/tau_cold_F*(desired_temp_cold_F/temp_cold_F - 1.0));

	for(int i = 0; i<natoms_F; i++){
        if(group_F[i] == 1){
		    vels_F[i][0] = vels_F[i][0]*lamda_hot;
            vels_F[i][1] = vels_F[i][1]*lamda_hot;
		    vels_F[i][2] = vels_F[i][2]*lamda_hot;
        }
        else{
            vels_F[i][0] = vels_F[i][0]*lamda_cold;
            vels_F[i][1] = vels_F[i][1]*lamda_cold;
		    vels_F[i][2] = vels_F[i][2]*lamda_cold;
        }
...
double percentage_hot = 50;								// Percentage of Hot particles. Remaining will be cold
double desired_temp_hot = 80.0;
double desired_temp_cold = 2.0;
double tau_hot = 0.01;
double tau_cold = 0.01;
double temp_hot = desired_temp_hot;
double temp_cold = desired_temp_cold;
double dt = 0.001;
double rcut_off = 4.0; //1.58113883;   // 2^(1/6)*sigma
int nsteps = 5000;
int output_freq = 10;
const char*  dump_File = "dump.lammpstrj";
const char*  input_File = "restart.jeet";					        // Input DATA File
const char*  output_Config_File = "finalConf.jeet";					// Output DATA File.
...
int main(int argc, char **argv){
  ...
  for (int i=1;i<argc;i++) {
    ...
    else if (!strcmp(argv[i],"-tau")) tau_hot=atof(argv[++i]);
    ...
    else if (!strcmp(argv[i],"-Th")) desired_temp_hot = atof(argv[++i]);
    else if (!strcmp(argv[i],"-Tc")) desired_temp_cold=atof(argv[++i]);
    ...
    tau_cold = tau_hot;   // Same time constant for hot and cold particles
	...
	FILE *outfile_Config;
        outfile_Config = fopen(output_Config_File, "w+");

	if(outfile_Dump==NULL || outfile_Config==NULL) { printf("Error::\t\n"); exit(0); } 
    ...
    FILE *infile;
     if(restart_flag == 1){ 
     	if(fopen(input_File, "r") == NULL){  printf("Error::\tRestart file %s do not exist!\n", input_File); exit(0); }
		...
		variable_Initialization(pos, vels, group, forces, box, mass, radius, natoms, dim, mass_atomic, rcut_off, radii,\
         ke_Energy_hot, desired_temp_hot, dt, epsilon, sigma, desired_temp_cold, ke_Energy_cold, n_hot_atoms);
    }
    ...
    while (step <= nsteps){
		...
		integrate(natoms, pos, vels, mass, forces, group, dt, rcut_off, epsilon, sigma, pot_Energy, box, pressure_virial, energy_correction, energy_at_cutoff);
				
		
        berendsen_thermostat(temp_hot, temp_cold, desired_temp_hot, desired_temp_cold, tau_hot, tau_cold, vels, group, dim, natoms, dt);
...
writeOutput(outfile_Config, step, natoms, dim, box, pos, vels, group);
